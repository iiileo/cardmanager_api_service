...
type UserQuery struct {
	...
	withOperatedLedgers *LedgerEntryQuery
...
// QueryOperatedLedgers chains the current query on the "operated_ledgers" edge.
func (_q *UserQuery) QueryOperatedLedgers() *LedgerEntryQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOperatedLedgers: _q.withOperatedLedgers.Clone(),
...
// WithOperatedLedgers tells the query-builder to eager-load the nodes that are connected to
// the "operated_ledgers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithOperatedLedgers(opts ...func(*LedgerEntryQuery)) *UserQuery {
	query := (&LedgerEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOperatedLedgers = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOperatedLedgers; query != nil {
		if err := _q.loadOperatedLedgers(ctx, query, nodes,
			func(n *User) { n.Edges.OperatedLedgers = []*LedgerEntry{} },
			func(n *User, e *LedgerEntry) { n.Edges.OperatedLedgers = append(n.Edges.OperatedLedgers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadOperatedLedgers(ctx context.Context, query *LedgerEntryQuery, nodes []*User, init func(*User), assign func(*User, *LedgerEntry)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ledgerentry.FieldOperatorID)
	}
	query.Where(predicate.LedgerEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OperatedLedgersColumn), fks...))
	}))
